...
- Use \`PIXEL_ASSETS.preloadPhaser(scene, requests)\` and \`PIXEL_ASSETS.createPhaserAnimations(scene, stableAssetId, animations?)\`.
- Resolve art by stable asset ID only. Never hardcode Supabase URLs or \`assets/*.png\` paths.
...
export const SYSTEM_PROMPT = `You build games using the **Atomic Game Maker** platform. Code lives as **atoms** (small JS functions, max 2KB) managed via MCP tools, not in files.
...
- \`read_externals(["three_js"])\` or \`read_externals(["phaser_js"])\` returns the full API surface -- always call this before writing code that uses an external library.
